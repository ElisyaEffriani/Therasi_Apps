...
function dateClickEvent(date) {
  ...
  fetch(
    "https://demo-tiger.vercel.app/oksigen/riwayat?date=" +
      new Date(Number.parseInt(date)).toDateString()
  )
    .then((res) => res.json())
    .then((res) => {
      console.log(res);
      generateTableData(res, ["timestamp", "data"]);
